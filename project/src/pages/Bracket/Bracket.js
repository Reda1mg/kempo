...
import Telecommande from '../Telecommande/components/Telecommande';
...
const Bracket = () => {
  ...
  const [scoreboardWindow, setScoreboardWindow] = useState(null);
  ...
  const startMatch = (matchId) => {
    // Démarrer le match en mode admin (télécommande sur la page principale)
    const match = matches.find(m => m.id === matchId);
    if (match) {
      setActiveMatch(match);
      
      // Ouvrir le scoreboard dans une nouvelle fenêtre
      const scoreboardUrl = `/scoreboard/${matchId}`;
      const newWindow = window.open(scoreboardUrl, '_blank', 'width=1200,height=800');
      setScoreboardWindow(newWindow);
    }
  };
  ...
  return (
    <div className="bracket-container">
      {/* Affichage de la télécommande admin si un match est actif */}
      {activeMatch && (
        <div className="admin-telecommande">
          <div className="admin-header">
            <h2>🎮 Télécommande Admin - Match en cours</h2>
            <button className="stop-match-btn" onClick={stopMatch}>
              ⏹️ Arrêter le match
            </button>
          </div>
          <Telecommande match={activeMatch} onBack={stopMatch} isEmbedded={true} />
        </div>
      )}

      {/* Affichage du bracket seulement si aucun match n'est actif */}
      {!activeMatch && (
        <>
          <div className="bracket-header">
            <h1>🏆 Bracket - {category?.name}</h1>
